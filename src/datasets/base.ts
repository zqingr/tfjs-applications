...
export class DataSet {
  ...
  loadImg (src: string): Promise<Float32Array> {
    return new Promise((resolve, reject) => {
      ...
      fs.readFile(process.cwd() + '/src' + src, (err, squid) => {
        if (err) throw err
        const img = new Image()
        img.onload = () => {
  ...
  async load () {
    ...
    this.trainLables = await JSON.parse(fs.readFileSync(process.cwd() + '/src' + this.TRAIN_LABLES, 'utf8'))
    this.testLables = await JSON.parse(fs.readFileSync(process.cwd() + '/src' + this.TEST_LABLES, 'utf8'))
